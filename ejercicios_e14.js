...
const catMuebles = productos.filter(c => c.categoria === "Muebles" );
console.log(catMuebles);

//3. Crea una función flecha que reciba el arreglo y devuelva el primer producto
//cuyo precio sea mayor a 1000.
const precioMayor = productos.find(p => p.precio > 1000);
...
const MayorA0 = productos.every(p => p.stock > 0);
console.log(MayorA0); // true
...
const categoriaE = productos.some(p => p.categoria === "Electrodomésticos");
console.log(categoriaE); // true
...
const menores10 = productos.filter(p => p.stock < 10);
